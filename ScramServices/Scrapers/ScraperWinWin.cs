...
namespace ScraperServices.Scrapers
{
    public class ScraperWinWin: ScraperBase
    {
        ...
        //private void _downloadItemsBySelenoid(List<string> list, ScraperWinWinStateModel state)
        //{
        //    _log($"Detect {list.Count} items with diferent format");
        ...
        //    {
        //        _downloadItemBySelenoid(itemId, selenoidState, state);
        //        var itemDto = _parseItemBySelenoid(itemId, selenoidState, state);
        //        _saveItemDtoAsync(itemDto, state).Wait();
        //    }

        //    _closeSelenoidBase(selenoidState, state);
        //}

        //private ItemWinWinDtoModel _parseItemBySelenoid(string itemId, SelenoidStateModel selenoidState, ScraperWinWinStateModel state)
        //{
        //    var coordinates = _parseItemBySelenoid_GetCoordinates(selenoidState, state);
        ...
        //private Tuple<string, string> _parseItemBySelenoid_GetCoordinates(SelenoidStateModel selenoidState, ScraperWinWinStateModel state)
        //{
        //    Thread.Sleep(1000 * 5);

        //    var mapElement1 = selenoidState.WindowMain.FindElementsByClassName("imgsMapButtonsBox");

        //    var mapElement = selenoidState.WindowMain.FindElementById("mapButton");

        //    mapElement.Click();

        //    selenoidState.WaitMain.Until(ExpectedConditions.ElementIsVisible(By.Id("iframeMapWrapper")));

        //    var networkJSScript = "var performance = window.performance || window.mozPerformance || window.msPerformance || window.webkitPerformance || {}; var network = performance.getEntries() || {}; return network;";
        //    string res1 = (string)selenoidState.WindowMain.ExecuteScript(networkJSScript);

        //    var result = new Tuple<string, string>("s1","s2");

        //    return result;
        //}

        //private void _downloadItemBySelenoid(string itemId, SelenoidStateModel selenoidState, ScraperWinWinStateModel state)
        //{
        //    _log($"Loading item-id (over Selenoid):{itemId}");
